...
use num::{ToPrimitive, rational::BigRational};
...
pub mod tests {
    ...
    use crate::checker::RunEgglogOptions;
    use crate::parser::{Config, Parser, parse_instance_with_pool};
    ...
    fn parse_term_with_definitions(
        ...
        let config = Config::new().apply_function_defs(true);
        let mut parser = Parser::new(pool, config, definitions.into()).expect("parser error");
        parser.parse_problem().expect("parse problem error");
        parser.reset(term_str.into()).expect("reset error");
