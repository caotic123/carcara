...
mod tests {
    ...
    fn test_uncrowd_resolution() {
        ...
        ";
        let (_, proof, mut pool) = parse_instance(problem, proof, None, parser::Config::new()).unwrap();
        ...
        ";
        let (_, expected, _) =
            parse_instance_with_pool(problem, expected, None, parser::Config::new(), &mut pool).unwrap();
