...
use carcara::{RunEgglogOptions, ast::ProofNode, parser, rare::engine::run_egglog};
use egglog::{
...
/// no e-node.
fn p_string(pattern: &Pattern, ops: &Interner, out: &mut Vec<PSymbol>) -> Option<()> {
    match pattern {
...
#[derive(Debug)]
struct DiscriminationTree {
    nodes: Vec<DtNode>,
}

#[derive(Debug, Default)]
struct DtNode {
    children: HashMap<PSymbol, u32>,
    /// Rule sides whose p-string ends at this node.
    entries: Vec<(usize, InstanceSide)>,
}

impl DiscriminationTree {
    fn build(rules: &[Rewrite], ops: &Interner) -> Self {
    ...
    fn insert(&mut self, string: &[PSymbol], entry: (usize, InstanceSide)) {
        let mut node = 0;
...
struct Reconstructor<'a> {
    ...
    /// rule list.
    pattern_index: DiscriminationTree,
...
fn node_with_root_id(
    commands: Vec<carcara::ast::ProofCommand>,
    root_id: &str,
) -> Option<carcara::ast::Rc<ProofNode>> {
    let forest = carcara::ast::ProofNodeForest::from_commands(commands);
    let mut found = None;
...
fn check_with_carcara(problem: &Path, steps: &[String], rare: &Path) -> Result<(), String> {
    ...
    .or_else(|error| match error {
        ...
        carcara::Error::DoesNotReachEmptyClause { .. } => Ok(()),
